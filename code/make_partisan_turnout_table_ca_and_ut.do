...
* Bring in the analysis data for California and Utah
...
* Prepare an analysis dataset with each row being a state-year-office (CA and UT only)
project, uses("$root/modified data/analysis.dta")
use "$root/modified data/analysis.dta", clear
keep if inlist(state, "CA", "UT")
...
quietly {
	cap log close
	set linesize 255
	log using "$root/output/partisan_effects_table_ca_and_ut.tex", text replace
	
	noi di "\begin{table}[h]"
	noi di "\centering"
	noi di "\caption{\textbf{Vote-by-Mail Expansion Does Not Appear to Favor Either Party (California and Utah).}\label{tab:partisan_effects_ca_and_ut}}"
