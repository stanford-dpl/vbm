...
* turnout share from tracts with >70% white residents
// Table S13, Col 1
reghdfe share_votes_high_white70 treat, ///
...
quietly {
	cap log close
	set linesize 255
	log using "$root/output/race_effects_table.tex", text replace
	
	noi di "\begin{table}[h]"
	noi di "\centering"
	noi di "\caption{\textbf{Vote-by-Mail Expansion Does Not Appear Have Large Effects on Share of the Electorate from Overwhelmingly-White Tracts.}\label{tab:race_effects}}"
	noi di "\begin{tabular}{lccc}"
	noi di "\toprule \toprule"
	noi di " & \multicolumn{3}{c}{Turnout Share High-White-Share Tracts [0-1]}  \\"
	noi di " & (1) & (2) & (3) \\"
	noi di "\midrule"
